...
class MapActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_back_map.setOnClickListener { this.finish() }
     ...
     * If Google Play services is not installed on the device, the icon_user will be prompted to install
     * it inside the SupportMapFragment. This method will only be triggered once the icon_user has
     * installed Google Play services and returned to the app.
     */
    override fun onMapReady(googleMap: GoogleMap) {
        mMap = googleMap

        val place = LatLng(38.170714, -115.343770)
        moveMap(place)                  //呼叫上面設好的方法來移動鏡頭
        addMarker(place, "食人迷宮", "玩家出生地no.23")
